...
opaque float (nat : Nat) (var_0 : List Nat) : const := by
  first
...
#eval ([1,2,3].map f).ap [4,5,6] |>.ap [7,8,9] |>.ap [10,11,12]
-- [22, 26, 30]

#eval [1,2,3].map (fun a => a + 1)

#eval a.map (fun w x y z => w + x + y + z) |>.ap b |>.ap c |>.ap d
