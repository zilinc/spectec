...
def list_ (X : Type) (var_0_opt : Option X) : List X :=
  match X, var_0_opt with
  | X, none => []
  | X, some w => [w]

def concat_ (X : Type) (var_0_lst_lst : List (List X)) : List X :=
  match X, var_0_lst_lst with
  | X, [] => []
  | X, [w_lst] ++ w'_lst_lst => w_lst ++ (concat_ X w'_lst_lst)

def disjoint_ (X : Type) (var_0_lst : List X) : Bool :=
  match X, var_0_lst with
  | X, [] => true
  | X, [w] ++ w'_lst => (! (List.contains w'_lst w)) && (disjoint_ X w'_lst)
...
inductive wf_uN : N → uN → Prop where
  | uN_case_0 (v_N : N) (i : Nat) : (i ≥ 0) && (i ≤ (cast ((cast (2 ^ v_N) Nat Nat) - (cast 1 Nat Nat)) Nat Nat)) → wf_uN v_N (.mk_uN i)
...
inductive wf_sN : N → sN → Prop where
  | sN_case_0 (v_N : N) (i : Nat) : (((i ≥ (- (cast (2 ^ (cast ((cast v_N Nat Nat) - (cast 1 Nat Nat)) Nat Nat)) Nat Nat))) && (i ≤ (- (cast 1 Nat Nat)))) || (i == (cast 0 Nat Nat))) || ((i ≥ (+ (cast 1 Nat Nat))) && (i ≤ ((cast (2 ^ (cast ((cast v_N Nat Nat) - (cast 1 Nat Nat)) Nat Nat)) Nat Nat) - (cast 1 Nat Nat)))) → wf_sN v_N (.mk_sN i)
...
inductive wf_fNmag : N → fNmag → Prop where
  | fNmag_case_0 (v_N : N) (v_m : m) (v_exp : exp) : (v_m < (2 ^ (fun_M v_N))) && ((((cast 2 Nat Nat) - (cast (2 ^ (cast ((cast (E v_N) Nat Nat) - (cast 1 Nat Nat)) Nat Nat)) Nat Nat)) ≤ v_exp) && (v_exp ≤ ((cast (2 ^ (cast ((cast (E v_N) Nat Nat) - (cast 1 Nat Nat)) Nat Nat)) Nat Nat) - (cast 1 Nat Nat)))) → wf_fNmag v_N (.NORM v_m v_exp)
  | fNmag_case_1 (v_N : N) (v_exp : exp) (v_m : m) : (v_m < (2 ^ (fun_M v_N))) && (((cast 2 Nat Nat) - (cast (2 ^ (cast ((cast (E v_N) Nat Nat) - (cast 1 Nat Nat)) Nat Nat)) Nat Nat)) == v_exp) → wf_fNmag v_N (.SUBNORM v_m)
...
def canon_ (v_N : N) : Nat :=
  match v_N with
  | v_N => 2 ^ (cast ((cast (Option.get! (signif v_N)) Nat Nat) - (cast 1 Nat Nat)) Nat Nat)
