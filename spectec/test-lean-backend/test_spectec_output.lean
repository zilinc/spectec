...
deriving Inhabited, BEq
     ...
     def instr_val (var_0 : val) : instr := match var_0 with
...
deriving Inhabited, BEq
     ...
     def «local» (v_state : state) (v_localidx : localidx) : val := match v_state with
| .mk_state (s) (f) => f.LOCALS[v_localidx]!

     def global (v_state : state) (v_globalidx : globalidx) : val := match v_state, v_globalidx with
| .mk_state (s) (f), x => s.GLOBALS[f.MODULE.GLOBALS[x]!]!

     def update_local (v_state : state) (v_localidx : localidx) (v_val : val) : state := match v_state, v_localidx, v_val with
| .mk_state (s) (f), x, v => .mk_state (s) ({
f with
LOCALS := List.modify (f.LOCALS) (x) (fun elem_1 => v)
})

     def update_global (v_state : state) (v_globalidx : globalidx) (v_val : val) : state := match v_state, v_globalidx, v_val with
