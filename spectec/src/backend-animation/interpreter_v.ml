...
and call_hostfunc name s vs =
  ...
  | "print_i32" when argc = 1 ->
    List.hd vs
    (* |> as_const "I32" *)
    (* |> vl_to_uN_32 *) |> Construct_v_new.vl_to_num |> (fun (I32 i) -> i)
    |> RI.I32.to_string_s
  ...
  | "print_i64" when argc = 1 ->
    List.hd vs
    (* |> as_const "I64" *)
    (* |> vl_to_uN_64 *) |> Construct_v_new.vl_to_num |> (fun (I64 i) -> i)
    |> RI.I64.to_string_s
  ...
  | "print_f32" when argc = 1 ->
    List.hd vs
    (* |> as_const "F32"
    |> vl_to_float32 *) |> Construct_v_new.vl_to_num |> (fun (F32 f) -> f)
    |> RI.F32.to_string
  ...
  | "print_f64" when argc = 1 ->
    List.hd vs
    (* |> as_const "F64"
    |> vl_to_float64 *) |> Construct_v_new.vl_to_num |> (fun (F64 f) -> f)
    |> RI.F64.to_string
  ...
  | "print_i32_f32" when argc = 2 ->
    let [v1; v2] = vs in
    let i32 = v1 |> (* as_const "I32" |> vl_to_uN_32 *) Construct_v_new.vl_to_num |> (fun (I32 i) -> i) |> RI.I32.to_string_s in
    let f32 = v2 |> (* as_const "F32" |> vl_to_float32 *) Construct_v_new.vl_to_num |> (fun (F32 f) -> f) |> RI.F32.to_string   in
    Printf.sprintf "- print_i32_f32: %s %s\n" i32 f32 |> print_eff |> fun e -> [e]
  | "print_f64_f64" when argc = 2 ->
    let [v1; v2] = vs in
    let f64  = v1 |> (* as_const "F64" |> vl_to_float64 *) Construct_v_new.vl_to_num |> (fun (F64 f) -> f) |> RI.F64.to_string in
    let f64' = v2 |> (* as_const "F64" |> vl_to_float64 *) Construct_v_new.vl_to_num |> (fun (F64 f) -> f) |> RI.F64.to_string in
